...
function M.report_startuptime()
  vim.api.nvim_create_autocmd("User", {
    ...
    callback = function()
      ...
      require("zaeberg.plugins.ui.alpha").update_startuptime(stats.loaded, stats.startuptime)
    end,
  })
end
...
function M.setup()
  ...
  local colorscheme = require("zaeberg.utils.config").colorscheme
  require("lazy").setup("zaeberg.plugins", {
    defaults = { lazy = true },
    concurrency = utils.get_os() == "mac" and 50 or nil,
    lockfile = vim.fn.expand("~/dotconfig/nvim/lua/zaeberg/lazy/lazy-lock.json"),
  ...
  require("lazy.view.config").dimmed_commits =
    { "build", "chore", "ci", "doc", "style", "refactor", "perf", "test", "fix", "highlight", "injection", "indents" }
end
