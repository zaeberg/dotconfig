...
function M.config()
  ...
  dashboard.section.buttons.val = {
    ...
    -- dashboard.button("f", "󰍉  Find Files", "<Cmd>lua require'zaeberg.plugins.ui.telescope'.file_search()<CR>"),
    ...
    -- dashboard.button("p", "󰏗  Find Projects", "<Cmd>Telescope projects<CR>"),
    -- dashboard.button("s", "  Restore Session", "<Cmd>SessionManager load_last_session<CR>"),
    -- dashboard.button("S", "󰍨  Find Sessions", "<Cmd>SessionManager load_session<CR>"),
    -- dashboard.button("c", "󰗹  Neovim Config", "<Cmd>lua require'zaeberg.plugins.ui.telescope'.dotconfig()<CR>"),
    -- dashboard.button("u", "  Update Plugins", "<Cmd>Lazy<CR>"),
    -- dashboard.button("q", "⏻  Quit Neovim", "<Cmd>qa<CR>"),
  }

  dashboard.section.footer.val = {
    "     https://github.com/zaeberg/",
...
function M.update_startuptime(loaded_plugins, startuptime)
  ...
  if ok then
    ...
    require("zaeberg.utils").notify("Failed to load dashboard from vim-startuptime", vim.log.levels.ERROR)
  end
end
