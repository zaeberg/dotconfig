...
local symbol_icons = require("zaeberg.utils.config").symbol_icons
...
local function clickable(content, fn_name)
  fn_name = "v:lua.require'zaeberg.winbar'." .. fn_name
...
function M.show_winbar()
  ...
  if not vim.tbl_contains(ignored_filetypes, filetype) then
    vim.api.nvim_win_set_option(win, "winbar", "%{%v:lua.require'zaeberg.winbar'.eval()%}")
  end
  focused_win = win
end
